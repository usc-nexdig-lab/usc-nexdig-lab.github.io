  ...
  return (
    <div className="fixed bg-white h-[60px] shadow w-full flex items-center space-x-[30px] px-[30px] py-[10px] text-sm tracking-wide z-50 text-gray-800 font-medium">
      ...
      >
        ...
        >
          <FontAwesomeIcon
            icon={faPeopleGroup}
            className={`nav-icon ${
              isActive('/people') ? 'text-blue-500' : ''
            } ${clickedIcon === 'people' ? 'clicked' : ''}`}
          />
          <h1
            className={`hidden sm:inline ${
              isActive('/people') ? 'text-blue-500' : ''
      ...
      >
        ...
        >
          <FontAwesomeIcon
            icon={faDigging}
            className={`nav-icon ${
              isActive('/research') ? 'text-blue-500' : ''
            } ${clickedIcon === 'research' ? 'clicked' : ''}`}
          />
          <h1
            className={`hidden sm:inline ${
              isActive('/research') ? 'text-blue-500' : ''
      ...
      >
        <button
          className="flex items-center space-x-[5px]"
          onClick={() => handleIconClick('publications')}
        >
          <FontAwesomeIcon
            icon={faNewspaper}
            className={`nav-icon ${
              isActive('/publications') ? 'text-blue-500' : ''
            } ${clickedIcon === 'publications' ? 'clicked' : ''}`}
          />
          <h1
            className={`hidden sm:inline ${
              isActive('/publications') ? 'text-blue-500' : ''
      ...
      >
        <button
          className="flex items-center space-x-[5px]"
          onClick={() => handleIconClick('opportunities')}
        >
          <FontAwesomeIcon
            icon={faFistRaised}
            className={`nav-icon ${
              isActive('/opportunities') ? 'text-blue-500' : ''
            } ${clickedIcon === 'opportunities' ? 'clicked' : ''}`}
          />
          <h1
            className={`hidden sm:inline ${
              isActive('/opportunities') ? 'text-blue-500' : ''
